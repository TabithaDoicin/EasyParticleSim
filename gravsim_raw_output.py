...
def initial_parameters(total_seconds, time_per_iteration, filename):
    global total_time, time_increment, iterations_total, file
...
def simulate_particles(*args):
    ...
    for l in range(7*len(args)):
        json.dump(D_O_L[l],file)
        file.write("\n")
    file.close()
    t1 = time()
    print("#####################Complete!#####################")
    print("Everything took " + str(t1-t0) + "s")
    print("List of positions, velocities, accelerations, and mass printed to file")        
